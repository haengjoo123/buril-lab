...
export const LABEL_SCAN_MAX_OUTPUT_TOKENS = 1_200
...
const reagentLabelSchema = z.object({
  fields: z.object({
    name: scanFieldSchema,
    casNumber: scanFieldSchema,
    capacity: scanFieldSchema,
    expiryDate: scanFieldSchema,
    manufacturerDateType: scanFieldSchema,
    brand: scanFieldSchema,
    productNumber: scanFieldSchema,
    containerType: scanFieldSchema,
  }),
})
...
- return only the JSON object`
    ...
    const result = await parseOpenAIResponse(context.env, {
      ...
      maxOutputTokens: LABEL_SCAN_MAX_OUTPUT_TOKENS,
      safetyIdentifier,
      schema: reagentLabelSchema,
      schemaName: 'reagent_label_scan',
    })

    return json(buildReagentLabelScanResponse(result.data))
  } catch {
    return json(
      {
        name: '',
        ...
        success: false,
        error: 'Failed to analyze reagent label.',
      },
      { status: 502 },
    )
  }
