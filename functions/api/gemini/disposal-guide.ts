...
const CACHE_VERSION = 'disposal_guide:v3'
...
export const onRequestPost = async (context: {
  ...
  const deterministic = buildStructuredDisposalGuide(input, null, {
    availability: context.env.GEMINI_API_KEY ? 'available' : 'unavailable',
    availabilityReason: context.env.GEMINI_API_KEY ? undefined : 'not_configured',
  ...
  if (cached?.schemaVersion === 3 && cached.aiResponse) {
  ...
  try {
    const result = await generateGeminiText(context.env.GEMINI_API_KEY, {
      contents: [{ role: 'user', parts: [{ text: buildPrompt(input, deterministic) }] }],
      generationConfig: {
        temperature: 0.1,
    ...
    const parsed = parseDisposalGuideJson(result.text)
    if (!parsed) throw new Error('Gemini returned invalid disposal-guide JSON.')
    ...
    console.warn('[Gemini Disposal Guide] Falling back to deterministic guidance:', error)
