...
/** Small synchronous SHA-256 implementation usable in both the browser and tests. */
export const sha256Text = (value: string): string => {
    ...
    for (let offset = 0; offset < bytes.length; offset += 64) {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            const first = (h! + bigSigma1 + choose + SHA256_ROUND_CONSTANTS[index]! + words[index]!) >>> 0;
...
export const normalizeLf = (value: string): string => value.replace(/\r\n?/g, '\n');
