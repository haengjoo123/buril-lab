...
상태: **2026-08-28 현재 시점 논리 복구훈련 완료 — 임시 프로젝트 삭제 확인**

이 문서는 운영 DB를 바꾸지 않고 별도 임시 프로젝트에서 복구 가능성을 확인하기 위한 실행 체크리스트입니다. 공개 가능한 실제 결과는 [2026-08-28 복구훈련 증거](./supabase-recovery-drill-evidence-2026-08-28.md)에 기록합니다. 체크박스가 비어 있으면 해당 세부 관문 완료 증거가 아닙니다.
...
- [x] 같은 지역, Micro, 24시간 안에 삭제라는 조건을 확인함
- [x] 생성 전 예상 Compute 비용이 1달러 이하임을 다시 확인함
- [x] 운영의 가장 최근 사용 가능한 백업 시각과 일일 백업 상태를 확인함
- [x] 작업자가 Supabase·GitHub·Cloudflare 계정 MFA를 사용함
- [x] 운영체제의 고정 절대경로에 있는 Microsoft 서명 64비트 Windows PowerShell 5.1만 OS 보호 상태 probe에 실행되고, Supabase CLI `2.115.0` 패키지·lock 무결성과 공식 portable PostgreSQL `17.11`의 69개 `pgsql/bin` 파일은 실행 없이 정적으로 검증됨
- [x] 공식 portable PostgreSQL 원본 압축파일의 실제 SHA-256이 `6EABDF00D2893713B75DB4336A23C3FDF505F056E217EC6E2E95D901750CFEA3`과 일치함
- [x] 복구 파일을 둘 BitLocker 보호 로컬 디렉터리 또는 EFS가 적용된 정확한 로컬 디렉터리의 절대경로와 작업 종료 시 안전하게 지울 방법을 준비함
- [x] EFS를 사용하면 작업 디렉터리 바로 아래 새 확인 파일에도 `Encrypted` 속성이 상속됐음을 읽기 전용으로 다시 확인함
- [x] 임시 디렉터리의 정규화된 절대경로가 저장소·OneDrive·다른 동기화 폴더 아래가 아님
- [x] 운영과 다른 Supabase 키, URL, 저장소, Redis, KV만 사용하도록 확인함
- [x] 외부 메일·예약 작업·삭제 worker·웹훅을 켜지 않는다는 확인문구를 기록함
- [x] 논리 덤프와 R2 파일을 같은 복구 시점으로 맞추기 위한 짧은 운영 쓰기·사진 업로드·삭제 중단 창을 별도로 승인받음
- [x] 최근 시약장 파일의 R2 `latest → complete → manifest → manifest.sha256` 연결과 실제 파일 본문이 존재함
- [x] 아래 읽기 전용 자동 사전점검이 통과함
...
2026-08-28에는 target 프로젝트 메타데이터와 정합 R2 완료 증거가 모두 생긴 뒤 자동 사전점검이 통과했고, 그 뒤에만 운영 DB 읽기와 덤프를 시작했습니다. 과거 준비 시점의 실패 상태는 위 기록으로 보존합니다.
...
- [x] 다섯 명령이 오류 없이 끝남
- [x] 백업 파일 5개의 SHA-256과 바이트 수를 비공개 작업 기록에 남김
- [x] 저장소나 클라우드 동기화 폴더에 백업 파일이 들어가지 않음
- [x] 운영 핵심 표의 행 수와 재고·폐기 익명 합계를 별도로 기록함
- [x] Vault 또는 별도 열 암호화를 사용하는지 확인함
- [x] 활성 예약 작업 수와 비밀 없는 설정 해시를 별도 기록함
- [x] 복원 직전에 5개 파일의 SHA-256을 다시 계산해 최초 값과 일치함
...
- [x] 사용자 비용 확인 ID를 받은 뒤에만 임시 프로젝트를 생성함
- [x] 소스와 같은 지역이며 Micro인지 확인함
- [x] 외부 메일 발송 설정을 연결하지 않음
- [x] Edge Function, Cron, 웹훅, Realtime publication을 켜지 않음
- [x] 앱 URL·redirect URL에 운영 주소를 넣지 않음
- [x] 운영 키·서비스 역할 키·JWT 비밀값을 복사하지 않음
- [x] 임시 프로젝트 접근자를 다시 확인함
- [x] 임시 프로젝트 ref를 운영·기존 Staging ref 금지 목록과 대조함
...
- [x] 한 트랜잭션으로 복원이 끝남
- [x] 오류를 무시하거나 일부만 수동 재실행하지 않음
- [x] custom login role이 있다면 target 전용 비밀번호를 별도 설정함
- [x] 새 표가 target의 넓은 기본 권한을 상속하지 않도록 공식 사전 REVOKE가 같은 트랜잭션에서 실행됨
- [x] 검토·해시 고정한 `auth-storage-changes.sql`이 같은 트랜잭션에서 적용됨
- [x] 일반 데이터 적재 직후 `session_replication_role`이 `origin`으로 돌아온 뒤 migration 이력과 `auth`·`storage` DDL이 적용됨
- [x] `auth`·`storage`의 사용자 trigger/RLS·정책이 source와 일치함
- [x] `supabase_migrations`의 이력 수와 해시가 source와 일치함
- [x] 외부 publication·웹훅·스케줄은 계속 OFF임
- [x] `cron.job`의 활성 작업이 0개임

복원이 실패하면 운영 DB에는 아무 조치도 하지 않습니다. 임시 프로젝트를 새로 만들기 전에 실패 원인, 경과 시간, 비용을 기록하고 재승인을 받습니다.

## 5. DB와 파일 검증

- [x] 운영과 복구 프로젝트의 핵심 표 행 수가 일치함
- [x] 재고 합계와 폐기 기록 합계가 일치함
- [x] 공개 스키마의 표·함수·정책·인덱스 차이를 확인함
- [x] RLS·GRANT 역할 시험이 통과함
- [x] Supabase Security Advisor 결과가 승인된 정확한 목록과 일치함
- [x] 운영 Storage 객체 본문은 DB 백업만으로 복원되지 않았음을 확인함
- [x] 비공개 R2 백업의 사진 manifest에서 표본 파일의 경로·크기·SHA-256을 확인함
- [x] 표본 사진을 격리된 target Storage에 복원하고 멤버십 회수 뒤 앱/DB RLS 조회가 0행임을 확인함
- [ ] 현재 운영 공개 URL을 비공개 경로와 짧은 서명 URL로 전환하는 운영 5 사진 Switch
...
- [x] 격리된 로컬 검증 웹이 임시 Supabase만 가리킴
- [x] 운영 Supabase·KV·Redis·R2 참조가 0건임
- [x] 로그인 성공
- [x] 기존 Auth 계정 로그인과 target 전용 검증 계정 로그인을 각각 확인함
- [x] 연구실 선택 성공
- [x] 재고 조회 성공
- [x] 폐액 기록 조회 성공
- [x] 직접 링크 조회 성공
- [x] 권한 있는 회원의 사진 조회 성공
- [x] 멤버십 회수 뒤 다른 연구실의 사진 메타데이터 조회가 RLS에서 0행임
- [x] 메일·예약 작업·삭제·외부 쓰기 0건
- [x] 전체 경과 시간이 4시간 이내임

## 7. 종료와 삭제

- [x] 익명 합계, 해시, 시작·종료 시각, 결과만 공개 가능한 증거 파일에 옮김
- [x] 공개 증거와 로그에 사용자 정보·파일 경로·비밀값이 없는지 확인함(백업 원문은 사용자 정보가 든 민감 자료로 간주)
- [x] 임시 프로젝트 삭제에 대한 별도 확인문구를 받음
- [x] 정확한 임시 프로젝트 ID를 재확인함
- [x] 임시 프로젝트를 생성 후 24시간 안에 삭제함
- [x] 삭제 완료 시각과 프로젝트 목록 제거를 확인함
- [x] 로컬 백업 파일과 연결 문자열을 지우고 임시 EFS 작업 디렉터리를 제거함(단순 파일 삭제를 복구 불가능한 안전 삭제로 주장하지 않음)
- [x] 작업 프로세스·브라우저 세션·Node 커널에서 비밀값을 제거하고 로컬 미리보기 listener 0건을 확인함
